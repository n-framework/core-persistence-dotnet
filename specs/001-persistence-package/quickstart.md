...
dotnet add package NFramework.Persistence.Generators
```
...
Create repository interfaces - the source generator will find these automatically:
...
using Microsoft.EntityFrameworkCore;
using MyApp.Data;
using NFramework.Persistence.Generated; // Auto-generated by the source generator
...
// Add all repositories - the source generator discovered them automatically
builder.Services.AddPersistenceRepositories();

var app = builder.Build();
```

That's it! The source generator found your repository interfaces and implementations, and registered them for you. No manual registration needed.
...
The in-memory database provider makes unit tests fast:
...
public class UserRepositoryTests
{
    ...
    public async Task DeleteAsync_PerformsSoftDelete()
    {
        ...
        // Not found in normal queries
        var notFound = await _repository.GetByIdAsync(user.Id);
        Assert.Null(notFound);

        // Found when including deleted
        var found = await _repository.GetByIdAsync(user.Id, includeDeleted: true);
...
### "No corresponding implementation class" Build Error

**Error:** `PG001: Repository interface 'IUserRepository' has no corresponding implementation class 'UserRepository'`

**Fix:** Make sure:

1. Your implementation class is named `UserRepository` (without the `I`)
2. The class implements `IUserRepository`
3. The class has a constructor that accepts `DbContext`

### DbContext Constructor Error

**Error:** `PG002: Repository implementation must have a constructor accepting DbContext`
